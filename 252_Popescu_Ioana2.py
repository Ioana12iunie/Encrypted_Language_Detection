...
transforma_prin_vectorizare = TfidfVectorizer(ngram_range=(1,13),analyzer = 'char')
#TfidfVectorizer are aceeasi functionalitate ca si CountVectorizer folosit precedent
#Insa construieste matricea tinand seama de frecventa termenilor
#De data aceasta nu am utilizat valoarea implicita a lui analyzer ('word'), ci 'char'
#Asadar, n gramele se vor construi pe baza de char uri, iar un range mai mare pentru n grame
#in urma testelor, ofera o acuratete mai buna
#am testat pentru mai multe valori posibile si ales varianta care dadea cel mai bine
#TfidfVectorizer dispune de functiile fit_transform si fit
...
#Voi importa al doilea model folosit
from sklearn.linear_model import SGDClassifier

a_doua_optiune = SGDClassifier(max_iter=1000000)
#am inclus in documentatie testele anterioare
#observatii: am ales o valoare mare pentru numarul maxim de iteratii
#pentru a evita ConvergenceWarning (imi aparuse pentru un caz)

#apelez fit pentru a aplica clasificatorul
a_doua_optiune.fit(antreneaza_pe, etichete_finale)

#dupa ce am efectuat antrenarea, apelez predict pe datele de test
#pentru a efectua predictia
predictie_pe_a_doua_optiune = a_doua_optiune.predict(testeaza_pe)
...
with open('submission.csv', 'w+') as sub_fis:
    ...
    for pred_id in ids_pentru_testare:
        ...
        sub_fis.write(',')
        sub_fis.write(str(predictie_pe_a_doua_optiune[pred_indx]))
        sub_fis.write('\n')
        pred_indx += 1
